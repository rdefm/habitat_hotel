...
Frame count for a convention-slot sheet is never fixed — it's the dropped-in file's own width/height, each divided by 64. 64×64 is both the on-screen footprint and the canonical per-frame size a convention-slot sheet is cut to — chosen to match the concept art already under `assets/` (`pigeon1.png`, `penguin1.png`, `tortoise.png`, all 64×64) and the `frame_w`/`frame_h` the sprite-generation tool that produced them already emits (see `metadata.json` inside e.g. `assets/pigeon1_walk.zip`). A sheet dropped at the conventional path is sliced into that many 64×64 frames straight off its own loaded pixel size — no frame count is assumed. `pigeon` and `penguin` are cut to this convention (`assets/characters/guests/<species_id>_<state>.png`, sourced from that same in-progress art-pipeline output) — a still `idle` for both, a `walk` cycle for both, plus `pigeon`'s own idle-animation clips (see Ambient idle animation, below). `penguin` has a still and a walk cycle but no idle clip yet — a legitimate, permanent case, not a gap: a character with no clip simply never animates while idle. No Species has `sleeping` art yet. The other six Species (`capybara`, `tortoise`, `flamingo`, `bat`, `polar_bear`, `snow_leopard`) have no art at all and render their placeholder for every state — the tortoise-themed art from that same pipeline drop is cut for Shelly (a Staffer) instead, not the `tortoise` Species, since it reads as her rather than as any tortoise guest (see below).

**Manny is the one alias**, not a convention-slot fit: his existing `assets/Manny-walk.png` (`walk`), `assets/Manny-sweeping.png` (`working`), and `assets/manny.png` (`idle`) — cut before this contract existed, at 256×256/256×256/76×76 per frame respectively — are fixed in `BuildingLayout.CHARACTER_SPRITE_FILE`, one-time the same way `ROOM_TYPE_INTERIOR_FILE` aliases the three painted Room interiors. They're scaled down to the same 64×64 footprint at render time.

**Shelly is a convention-slot fit**, not an alias: the tortoise-themed art from the same in-progress pipeline drop that gave `pigeon`/`penguin` their guest art is cut instead to `assets/characters/staffers/shelly_<state>.png` (`idle`, `walk`, plus her own `shelly_idle_1.png` ambient clip, see below) — Shelly reads as that tortoise, so her art lives on her own Staffer slot rather than on the `tortoise` Species' guest slot, which now falls back to the placeholder like every other unpainted Species. She has no `working` sheet yet, so that state falls through to the placeholder same as any other Staffer's would.

Both families' animation states are idle, walk, and one context state — sleeping for a Guest at Night, working for a Staffer at their Station — per ADR-0015/ADR-0020. There is no wander behaviour, but see Ambient idle animation below for the one exception to "no ambient behaviour scheduler."

### Ambient idle animation

A character standing on its plain `idle` still doesn't stay frozen forever: `ui/idle_animator.gd` wraps the `CharacterSprite` for a Staffer at rest and a settled Room occupant by day, and every few seconds (randomized, `Rng` per the determinism rule) swaps to one of that character's idle-animation clips, plays it once, then returns to the still. `sim/building_layout.gd`'s `resolve_character_idle_clips()` names which clips (if any) exist, checked in the same two-step order as `resolve_character_sprite()`:

1. `BuildingLayout.CHARACTER_IDLE_CLIP_FILE`'s one-time alias — Manny's `assets/manny-idle.png` (82×82 per frame) today, the same non-conventional-size reasoning his walk/working sheets get aliased for.
2. The open numbered convention `assets/characters/<kind>s/<id>_idle_<n>.png` for `n = 1, 2, …`, probed in order until one is missing — `pigeon` has two (`pigeon_idle_1.png`, `pigeon_idle_2.png`, picked between at random), Shelly (a Staffer) has one (`shelly_idle_1.png`).

A character with neither — `penguin`, Biscuit, and every Species still on the fallback placeholder (including `tortoise`, whose real art now belongs to Shelly instead) — simply never animates while idle; the still (or placeholder) just holds. Not wired into lobby guests or Terrace diners: `ui/hotel_world.gd` fully destroys and recreates those actors on essentially every frame (their own Patience decay changes the rebuild signature every tick, per tickets 09/10), which would reset the clip timer before it ever fired — they render their still frame only, same as every other placeholder-era simplification in this contract, until a future pass gives their rebuild loop a stable per-guest identity to hang this state off instead.
...
A Staffer always renders their `idle` state at rest here (see Ambient idle animation, above, for the still-plus-occasional-clip behaviour that state now carries) — nothing yet drives a `working` state, since that means being mid-Job (Housekeeping/Kitchen), which tickets 08/13's Job travel adds.
