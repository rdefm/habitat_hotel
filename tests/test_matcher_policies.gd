...
## (strict_match / fill_vacancies) before this policy-branching logic is
## deleted entirely in favor of manual seating -- see ADR-0001 and
...
func _room(slot: int, occupant: Variant = null, needs_cleaning: bool = false) -> Dictionary:
	return {"slot": slot, "room_type_id": "test_room", "occupant": occupant, "needs_cleaning": needs_cleaning}


func test_strict_match_admits_a_guest_whose_needs_are_fully_covered() -> void:
	var arrival := _arrival(["cold", "water"], 2)
	var hotel_rooms := [_room(0)]
	var room_stats := {0: {"capacity": 2, "tags": ["cold", "water"]}}
	...
	assert_eq(decision["room_slot_index"], 0)
...
func test_strict_match_turns_away_a_guest_when_only_a_mismatched_vacancy_exists() -> void:
	var arrival := _arrival(["cold", "water"], 2)
	var hotel_rooms := [_room(0)]
	var room_stats := {0: {"capacity": 2, "tags": ["warm", "water"]}} # missing "cold"
	var price_multipliers := {"test_room": 1.0}
	...
	assert_eq(decision["room_slot_index"], -1)
...
func test_both_policies_turn_a_guest_away_when_every_room_is_occupied() -> void:
	var arrival := _arrival(["cold", "water"], 2)
	var hotel_rooms := [_room(0, 42)] # occupied
	var room_stats := {0: {"capacity": 2, "tags": ["cold", "water"]}}
	var price_multipliers := {"test_room": 1.0}
