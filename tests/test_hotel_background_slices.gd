...
## sky/city layer plus the seven full-width band slices, stacked back in
## their original order, reconstruct the source painting. A small per-byte
## tolerance absorbs JPEG-decoder rounding between Godot's and the slicing
## tool's decode of the same source file -- it would not absorb an actually
## missing or misplaced band.
...
func test_sliced_bands_reconstruct_the_source_painting() -> void:
	...
	assert_eq(covered_height, source.get_height(), "the seven bands should cover the full canvas height with no gap or overlap")
...
func test_repeating_slots_tile_with_no_seam_across_eight_bands() -> void:
