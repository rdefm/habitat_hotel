...
## empty, Manny unassigned in the Staff Pool (ADR-0019 removed the Bellhop
## Station he used to start on). Skills (data/staffers.json): Biscuit R5 H2
## K1, Manny R2 H3 K3, Shelly R1 H5 K2.
...
func test_stacking_onto_a_room_with_no_in_flight_job_is_rejected() -> void:
	...
	assert_false(Sim.can_stack_staffer_on_room("manny", "roost_loft", 0))
	assert_false(Sim.stack_staffer_on_room("manny", "roost_loft", 0))
	assert_eq(GameState.staffer_station("manny"), "", "a rejected stack shouldn't move the Staffer's Station")
	assert_true(Sim.cleaning_job("manny").is_empty())
...
func test_stacking_onto_an_in_progress_room_sums_skill_and_looks_up_combined_ticks() -> void:
	...
	assert_true(Sim.can_stack_staffer_on_room("manny", "cozy_nook", 0))
	var stacked := Sim.stack_staffer_on_room("manny", "cozy_nook", 0)

	assert_true(stacked)
	assert_eq(GameState.staffer_station("manny"), "housekeeping", "stacking pulls the dragged Staffer onto the Station too")
	var expected: int = HK_TICKS_BY_SKILL[str(mini(5 + 3, 5))] # Shelly 5 + Manny 3, capped at 5
	assert_eq(int(Sim.cleaning_job("manny")["ticks_remaining"]), expected)
	assert_eq(int(Sim.cleaning_job("shelly")["ticks_remaining"]), expected)
	assert_eq(Sim.cleaning_job("manny")["room_type_id"], "cozy_nook")
...
func test_stacking_sums_skill_rather_than_taking_the_max_when_under_the_cap() -> void:
	...
	GameState.staffers["manny"]["skills"]["housekeeping"] = 2
	GameState.room_instance("cozy_nook", 0)["needs_cleaning"] = true
	Clock.force_advance_ticks(1) # Shelly claims cozy_nook at (edited) skill 1

	Sim.stack_staffer_on_room("manny", "cozy_nook", 0)
...
func test_a_third_staffer_dropped_onto_a_full_housekeeping_job_is_rejected() -> void:
	...
	Sim.stack_staffer_on_room("manny", "cozy_nook", 0) # now 2 Staffers deep
	var manny_ticks_before: int = int(Sim.cleaning_job("manny")["ticks_remaining"])
	...
	assert_eq(int(Sim.cleaning_job("manny")["ticks_remaining"]), manny_ticks_before)
...
func test_stacking_onto_a_different_room_abandons_the_staffers_own_job() -> void:
	Sim.assign_staffer("manny", "housekeeping")
	GameState.room_instance("cozy_nook", 0)["needs_cleaning"] = true
	GameState.room_instance("roost_loft", 0)["needs_cleaning"] = true
	Clock.force_advance_ticks(1) # Shelly and Manny each claim a different Room

	var manny_job_before := Sim.cleaning_job("manny")
	assert_eq(manny_job_before["room_type_id"], "roost_loft")

	Sim.stack_staffer_on_room("manny", "cozy_nook", 0) # drag Manny onto Shelly's Room instead

	assert_eq(Sim.cleaning_job("manny")["room_type_id"], "cozy_nook")
	assert_true(GameState.room_instance("roost_loft", 0)["needs_cleaning"], "Manny's abandoned Room stays dirty")
...
func test_stacking_onto_an_in_progress_breakfast_entry_sums_skill_and_looks_up_combined_ticks() -> void:
	...
	assert_true(Sim.can_stack_staffer_on_breakfast("manny", entry_id))
	var stacked := Sim.stack_staffer_on_breakfast("manny", entry_id)

	assert_true(stacked)
	assert_eq(GameState.staffer_station("manny"), "kitchen")
	var expected: int = BREAKFAST_TICKS_BY_SKILL[str(mini(2 + 3, 5))]
	assert_eq(int(Sim.breakfast_job("manny")["ticks_remaining"]), expected)
	assert_eq(int(Sim.breakfast_job("shelly")["ticks_remaining"]), expected)
	assert_eq(int(Sim.breakfast_job("manny")["entry_id"]), entry_id)
...
func test_stacking_onto_a_breakfast_entry_with_no_in_flight_job_is_rejected() -> void:
	...
	assert_false(Sim.can_stack_staffer_on_breakfast("manny", entry_id))
	assert_false(Sim.stack_staffer_on_breakfast("manny", entry_id))
...
func test_a_third_staffer_dropped_onto_a_full_breakfast_job_is_rejected() -> void:
	...
	var entry_id: int = int(Sim.breakfast_job("shelly")["entry_id"])
	Sim.stack_staffer_on_breakfast("manny", entry_id)
...
func test_stacking_onto_an_in_progress_dinner_entry_sums_skill_and_looks_up_combined_ticks() -> void:
	Sim.assign_staffer("manny", "kitchen") # skill 3, at dinner_min_skill
	_push_walkin(1, 999.0)
	Clock.force_advance_ticks(1) # Manny claims the Walk-in Diner
	var entry_id: int = int(Sim.dinner_job("manny")["entry_id"])
	...
	assert_eq(int(Sim.dinner_job("manny")["ticks_remaining"]), expected)
