...
##
## Staffers are freely reassignable across the three Stations (ADR-0005) via
...
## rate and Housekeeping's per-Staffer cleaning jobs (_tick_housekeeping()).
## Check-in itself is no longer one of those effects: ADR-0019 removed the
## Bellhop Station and its Escort Job, so a seated Party always walks to its
## Room over the same flat, data-driven delay (checkin.delay_ticks)
## regardless of staffing -- see _start_checkin()/_tick_checkins(). Kitchen
...
## A seated Party doesn't land in its Room instantly: it spends a flat,
## data-driven delay (balance.json's checkin.delay_ticks) walking there,
## counted down by _tick_checkins(). No Station gates this -- ADR-0019
## removed the Bellhop Station and its Skill-scaled Escort Job, leaving one
## check-in path for every Party rather than a staffed and an unstaffed one.
func _start_checkin(room: Dictionary) -> void:
	room["checking_in"] = true
	room["checkin_ticks_remaining"] = int(GameState.balance.get("checkin", {}).get("delay_ticks", 0))


## The one check-in path there is -- see _start_checkin().
