...
Verification: `tests/test_hotel_background_slices.gd` (GUT, 4 tests) — composites sky_and_city + the roofline cap + the five original bands + the ground plinth and diffs against the source (bit-exact in practice; tolerance only guards against cross-decoder rounding); separately asserts the sky layer is transparent at a point deep inside the solid building and a building slice is transparent in its own sky-margin, so "no building baked into sky / no sky baked into building" is checked directly rather than only implied by a reconstruction that can't distinguish a real punch-out from a naive fully-opaque cut; asserts each repeating slot has real (non-transparent) content at both its top and bottom edge, which is what makes 8x stacking gap-free (the *visual* seam judgment stays a manual check — 8x vertical stacks of both columns and the shaft, and 3-/8-floor generic composites — described here, not a checked-in fixture); and asserts the roof-cap-to-floor-row width equality, which holds at any floor count since width doesn't vary with height, covering the ticket's "at three and at eight" language with one relationship instead of two separate stack scenarios.
